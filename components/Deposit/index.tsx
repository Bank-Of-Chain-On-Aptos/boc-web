...
import { ACTION, APTI_COIN_DECIMALS, APTI_COIN_TYPE, APT_COIN_DECIMALS, APT_COIN_TYPE, APT_VAULT_ADDRESS, APT_VAULT_NAME } from '@/constant'
import { getAptosInstance } from '@/util/aptos'
import { Account, Ed25519PrivateKey } from '@aptos-labs/ts-sdk'
import { getEstimatedOutputAmount, submitTxByPayer } from '@/service/bocApi'
...
const Deposit = () => {
  ...
  const decimals = APT_COIN_DECIMALS
  const minAmount = 1 / decimals

  const fromCoinFetcher = useMyCoinBalance(APT_COIN_TYPE, APT_COIN_DECIMALS)
  const toCoinFetcher = useMyCoinBalance(APTI_COIN_TYPE, APTI_COIN_DECIMALS)
  ...
  const deposit = async () => {
    // check input amount
    if (!inputAmount || inputAmount <= minAmount || inputAmount > fromCoinFetcher.balance) {
      return
    }
    // calculate deposit amount
    const depositAmount = inputAmount * decimals
    ...
    const txBuildData = {
      ...
      data: {
        function: `${APT_VAULT_ADDRESS}::${APT_VAULT_NAME}::deposit`,
        typeArguments: [APT_COIN_TYPE],
        functionArguments: [depositAmount.toFixed()]
      }
    }
    const transaction = await aptos.transaction.build.simple(txBuildData)
    ...
    // submitTxByPayer(txBuildData, senderAuthenticator).then(async (data) => {
    //   if (data.status === 200) {
    //     openNotification('Congradulation! Your transaction has been submitted.', 'success')
    //   } else {
    //     openNotification('Sorry, we met some errors...', 'error')
    //   }
    //   setInputAmount(0)
    //   setOutputAmount(0)
  ...
  useEffect(() => {
    const fetchOutputAmount = async () => {
      ...
      const { estimatedOutputAmount } = await getEstimatedOutputAmount(APTI_COIN_TYPE, ACTION.DEPOSIT, inputAmount)
  ...
  return (
    <>
      ...
      <div className={s.bottom}>
        <p>To</p>
        <div className={s.row}>
          <span className="grow">{isUsdi ? 'USD' : 'APT'}i</span>
